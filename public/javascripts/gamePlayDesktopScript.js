...
socket.on('moveThePlayer', function(msg){
  console.log(`player 1 id: ${msg.playerID}`);
  console.log(`and this is data of player id: ${dataOfPlayer[0].playerId}`);
  readyPlayerOne(msg);
...
function readyPlayerOne(msg){
  ctx.globalCompositeOperation = 'destination-over';
  ctx.clearRect(0, 0, 900, 600);

  ctx.save();
  progressPlayer1 += 1;
  if (msg.id === dataOfPlayer[0].playerId) {
...
function readyPlayerTwo(){
  ...
  if ( xPosition2 < 680 && yPosition2 == 340 ) {
    console.log('bottom Row 2');
    xPosition2 += 0.5;
    ctx2.drawImage(player2Char, xPosition2, yPosition2);
  }else if(yPosition2 > 250){
    ...
    ctx2.translate(xPosition2, yPosition2);

    ctx2.drawImage(player2Char, 0, 0);

    ctx2.restore();

  }else if(xPosition2 > 150 && yPosition2 >= 250){
    ...
    ctx2.drawImage(player2Char, xPosition2, yPosition2);
    ctx2.restore();
  }else if(xPosition2 < 680 && yPosition2 >= 10){
    // LEFT CURVE
